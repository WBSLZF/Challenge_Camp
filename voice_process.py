...
def read_data():
    # 数据完整无na
    names_voice = ['uid', 'tel_opp_num', 'tel_opp_head', 'tel_opp_len', 'tel_start_time', 'tel_end_time',
                   'tel_call_type', 'tel_in_and_out']
    voice_train = pd.read_csv("../data/train/voice_train.txt", header=None, encoding='utf-8', names=names_voice,
                              ...
                              low_memory=False)
    voice_test = pd.read_csv("../data/test/voice_test_a.txt", header=None, encoding='utf-8', names=names_voice,
                             index_col=False, delimiter='\t',
                             dtype={
                                 'tel_start_time': int,
                                 'tel_opp_num': str,
    ...
    voice_data['tel_len'] = (((voice_data['tel_end_time'] // 1e6 - voice_data['tel_start_time'] // 1e6) * 24 + \
                              (voice_data['tel_end_time'] // 1e4 % 100 - voice_data[
                                  'tel_start_time'] // 1e4 % 100)) * 60 + \
                             (voice_data['tel_end_time'] // 1e2 % 100 - voice_data[
                                 'tel_start_time'] // 1e2 % 100)) * 60 + \
...
def get_tel_date_feature(voice_data):
    ...
    tel_in_date_static = voice_data[voice_data['tel_in_and_out'] == 1].groupby(['uid', 'tel_date'])[
        'uid'].count().groupby(['uid']).agg(
        ['mean', 'count', 'sum', 'std']).add_prefix('tel_in_date_')
    tel_out_date_static = voice_data[voice_data['tel_in_and_out'] == 0].groupby(['uid', 'tel_date'])[
        'uid'].count().groupby(['uid']).agg(
    ...
    tel_date_static['tel_date_in_out_diff_mean'] = tel_date_static['tel_in_date_mean'] - tel_date_static[
        'tel_out_date_mean']
    tel_date_static['tel_date_in_out_diff_count'] = tel_date_static['tel_in_date_count'] - tel_date_static[
        'tel_out_date_count']
    tel_date_static['tel_date_in_out_diff_sum'] = tel_date_static['tel_in_date_sum'] - tel_date_static[
        'tel_out_date_sum']
    tel_date_static['tel_date_in_out_diff_std'] = tel_date_static['tel_in_date_std'] - tel_date_static[
        'tel_out_date_std']
    ...
    tel_date_all_kinds = voice_data.groupby(['uid', 'tel_date'])['tel_date'].count().unstack('tel_date').add_prefix(
        'tel_date_')
    tel_in_date_all_kinds = voice_data[voice_data['tel_in_and_out'] == 1].groupby(['uid', 'tel_date'])[
        'tel_date'].count().unstack('tel_date').add_prefix('tel_in_date_')
    tel_out_date_all_kinds = voice_data[voice_data['tel_in_and_out'] == 0].groupby(['uid', 'tel_date'])[
        'tel_date'].count().unstack('tel_date').add_prefix('tel_out_date_')
...
def get_tel_opp_head_catagory(voice_data):
    tel_opp_head_catagory = voice_data.groupby(['uid', 'tel_opp_head'])['tel_opp_head'].count().unstack(
        'tel_opp_head').add_prefix('tel_opp_head_cat_')
...
def get_tel_opp_num_feature(voice_data):
    short_num_kind = voice_data[(voice_data['tel_opp_len'] <= 5)].groupby(['uid', 'tel_opp_num'])[
        'tel_opp_num'].count().unstack('tel_opp_num').add_prefix('tel_opp_num_')
    # long kind试过， 单表分数下降
    ...
    for name in hot_tel_num:
        hot_tel_opp_num_cnt.append(
            voice_data[voice_data['tel_opp_num'] == name].groupby(['uid'])['uid'].agg(['count']).add_prefix(
                'hot_tel_opp_num_' + name + '_cnt_'))

    hot_tel_opp_num_cnt.append(voice_data[voice_data['tel_opp_num'].apply(
        lambda x: x not in hot_tel_num)].groupby(['uid'])['uid'].agg(['count']).add_prefix(
        'other_tel_opp_num_' + '_cnt_'))
...
def get_tel_type_catagory(voice_data):
    ...
    tel_out_type_category = voice_data[voice_data['tel_in_and_out'] == 0].groupby(
        ['uid', 'tel_call_type'])['tel_call_type'].count().unstack('tel_call_type')
    tel_out_type_category.columns = list(
        map(lambda x: 'tel_type_' + str(x) + '_fre_out', tel_out_type_category.columns))
...
def get_tel_len_div(voice_data):
    ...
    tel_mid_fre = voice_data[(voice_data['tel_len'] > 120) & (voice_data['tel_len'] <= 600)].groupby(['uid'])[
        'uid'].count()  # 2-10min 次数
...
def get_tel_opp_len_div(voice_data):
    ...
    tel_opp_less_11 = voice_data[(voice_data['tel_opp_len'] < 11) & (voice_data['tel_opp_len'] > 5)].groupby(['uid'])[
        'uid'].count()
...
def get_tel_opp_len_catagory(voice_data):
    tel_opp_len_all_kinds = voice_data.groupby(['uid', 'tel_opp_len'])['uid'].count().unstack().add_prefix(
        'tel_opp_len_')
    tel_out_opp_len_all_kinds = voice_data[voice_data['tel_in_and_out'] == 0].groupby(['uid', 'tel_opp_len'])[
        'uid'].count().unstack().add_prefix('tel_out_opp_len_')
    tel_in_opp_len_all_kinds = voice_data[voice_data['tel_in_and_out'] == 1].groupby(['uid', 'tel_opp_len'])[
        'uid'].count().unstack().add_prefix('tel_in_opp_len_')
    # 可以做一个in_out_diff,还没写。
    tel_opp_len_all_kinds = pd.concat([tel_opp_len_all_kinds, tel_out_opp_len_all_kinds, tel_in_opp_len_all_kinds],
                                      axis=1)
...
def get_tel_hour_feature(voice_data):
    ...
    tel_hour_div_1 = voice_data[(voice_data['tel_hour'] > 8) & (voice_data['tel_hour'] <= 12)].groupby(['uid'])[
        'uid'].count()
    tel_hour_div_2 = voice_data[(voice_data['tel_hour'] > 12) & (voice_data['tel_hour'] <= 14)].groupby(['uid'])[
        'uid'].count()
    tel_hour_div_3 = voice_data[(voice_data['tel_hour'] > 14) & (voice_data['tel_hour'] <= 17)].groupby(['uid'])[
        'uid'].count()
    ...
    tel_in_hour_div_1 = \
    voice_in_data[(voice_in_data['tel_hour'] > 8) & (voice_in_data['tel_hour'] <= 12)].groupby(['uid'])['uid'].count()
    tel_in_hour_div_2 = \
    voice_in_data[(voice_in_data['tel_hour'] > 12) & (voice_in_data['tel_hour'] <= 14)].groupby(['uid'])['uid'].count()
    tel_in_hour_div_3 = \
    voice_in_data[(voice_in_data['tel_hour'] > 14) & (voice_in_data['tel_hour'] <= 17)].groupby(['uid'])['uid'].count()
    tel_in_hour_div_4 = voice_in_data[voice_in_data['tel_hour'] >= 17].groupby(['uid'])['uid'].count()
    tel_in_hour_div = pd.concat(
        [tel_in_hour_div_0, tel_in_hour_div_1, tel_in_hour_div_2, tel_in_hour_div_3, tel_in_hour_div_4], axis=1)
    tel_in_hour_div.columns = ['tel_in_hour_div_0', 'tel_in_hour_div_1', 'tel_in_hour_div_2', 'tel_in_hour_div_3',
                               'tel_in_hour_div_4']
    ...
    tel_out_hour_div_1 = \
    voice_out_data[(voice_out_data['tel_hour'] > 8) & (voice_out_data['tel_hour'] <= 12)].groupby(['uid'])[
        'uid'].count()
    tel_out_hour_div_2 = \
    voice_out_data[(voice_out_data['tel_hour'] > 12) & (voice_out_data['tel_hour'] <= 14)].groupby(['uid'])[
        'uid'].count()
    tel_out_hour_div_3 = \
    voice_out_data[(voice_out_data['tel_hour'] > 14) & (voice_out_data['tel_hour'] <= 17)].groupby(['uid'])[
        'uid'].count()
    tel_out_hour_div_4 = voice_out_data[voice_out_data['tel_hour'] >= 17].groupby(['uid'])['uid'].count()
    tel_out_hour_div = pd.concat(
        [tel_out_hour_div_0, tel_out_hour_div_1, tel_out_hour_div_2, tel_out_hour_div_3, tel_out_hour_div_4], axis=1)
    tel_out_hour_div.columns = ['tel_out_hour_div_0', 'tel_out_hour_div_1', 'tel_out_hour_div_2', 'tel_out_hour_div_3',
                                'tel_out_hour_div_4']
...
def get_tel_other_feature(voice_data):
    voice_opp_num_unique = voice_data.groupby(['uid'])['tel_opp_num'].agg(
        {'unique_count': lambda x: len(pd.unique(x))}).add_prefix('voice_opp_num_')
    voice_opp_head_unique = voice_data.groupby(['uid'])['tel_opp_head'].agg(
        {'unique_count': lambda x: len(pd.unique(x))}).add_prefix('voice_opp_head_')
    voice_opp_len_all_kinds = voice_data.groupby(['uid', 'tel_opp_len'])['uid'].count().unstack().add_prefix(
        'voice_opp_len_')
    voice_opp_len_all_kinds.fillna(0, inplace=True)

    tel_len_static = voice_data.groupby(['uid'])['tel_len'].agg(
        ['std', 'max', 'min', 'median', 'mean', 'sum']).add_prefix('tel_len_')
    # tel_in_len_static导致分数下降，可能是过拟合。先加了再说。。。
    tel_in_len_static = voice_data[voice_data['tel_in_and_out'] == 1].groupby(['uid'])['tel_len'].agg(
        ['std', 'max', 'min', 'median', 'mean', 'sum']).add_prefix('tel_len_in_')
    tel_out_len_static = voice_data[voice_data['tel_in_and_out'] == 0].groupby(['uid'])['tel_len'].agg(
        ['std', 'max', 'min', 'median', 'mean', 'sum']).add_prefix('tel_len_out_')
    tel_len_static = pd.concat([tel_len_static, tel_in_len_static, tel_out_len_static], axis=1)

    tel_len_date_static = voice_data.groupby(['uid', 'tel_date'])['tel_len'].sum().groupby(['uid']).agg(
        ['std', 'max', 'min', 'median', 'mean', 'sum']).add_prefix('tel_len_date_')
    ...
    other_tel_data = voice_data[
        voice_data['tel_opp_head'].apply(lambda x: x not in (yidong_head + liantong_head + dianxin_head))]

    yidong_tel_static = yidong_tel_data.groupby(['uid'])['tel_len'].agg(['std', 'sum', 'max', 'mean']).add_prefix(
        'yidong_tel_len_')
    liantong_tel_static = liantong_tel_data.groupby(['uid'])['tel_len'].agg(['std', 'sum', 'max', 'mean']).add_prefix(
        'liantong_tel_len_')
    dianxin_tel_static = dianxin_tel_data.groupby(['uid'])['tel_len'].agg(['std', 'sum', 'max', 'mean']).add_prefix(
        'dianxin_tel_len_')
    other_tel_static = other_tel_data.groupby(['uid'])['tel_len'].agg(['std', 'sum', 'max', 'mean']).add_prefix(
        'other_tel_len_')
    tel_operator_div = pd.concat([yidong_tel_static, liantong_tel_static, dianxin_tel_static, other_tel_static], axis=1)

    tel_operator_fre = pd.concat(
        [yidong_tel_data.groupby(['uid'])['uid'].count(), liantong_tel_data.groupby(['uid'])['uid'].count(),
...
def get_voice_feature_matrix():
    ...
    tel = pd.concat(
        [tel_in_and_out, tel_type_category, tel_len_mean, tel_len_sum, tel_len_max, tel_len_div, tel_opp_len_div,
         others,
...
def local_train_and_test(tel):
    ...
    tel_local_train_X, tel_local_test_X, local_train_Y, local_test_Y = model_selection.train_test_split(tel_X_train,
                                                                                                        tel_y_train,
                                                                                                        test_size=0.25,
                                                                                                        random_state=42)
